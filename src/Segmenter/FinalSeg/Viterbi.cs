...
namespace JiebaNet.Segmenter.FinalSeg
{
    public class Viterbi : IFinalSeg
    {
        ...
        public IEnumerable<string> Cut(string sentence)
        {
            ...
            foreach (string blk in RegexChinese.Split(sentence))
            {
                ...
                {
                    IEnumerable<string> segments = RegexSkip.Split(blk).Where(seg => !string.IsNullOrEmpty(seg));
        ...
        private void LoadModel()
        {
            ...
            string transJson = File.ReadAllText(Path.GetFullPath(ConfigManager.ProbTransFile));
            _transProbs = JsonConvert.DeserializeObject<IDictionary<char, IDictionary<char, double>>>(transJson);

            string emitJson = File.ReadAllText(Path.GetFullPath(ConfigManager.ProbEmitFile));
        ...
        {
            List<IDictionary<char, double>> v = new List<IDictionary<char, double>>();
            ...
            for (int i = 1; i < sentence.Length; ++i)
            {
                ...
                foreach (char y in States)
                {
                    ...
                    foreach (char y0 in _prevStatus[y])
                    {
                        double tranp = _transProbs[y0].GetDefault(y, Constants.MinProb);
            ...
            Node finalPath = probE < probS ? path['S'] : path['E'];
